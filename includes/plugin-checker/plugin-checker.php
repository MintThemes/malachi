...
if ( !class_exists( 'MP_CORE_Plugin_Checker' ) ){
	class MP_CORE_Plugin_Checker{
		...
		public function mp_core_update_check_callback() {
			...
			add_action( 'admin_footer', array( $this, 'mp_core_remove_check_notice')  );
		...
		/**
		 * Function to remove notices from admin for plugins
		 */
		function mp_core_remove_check_notice(){
				
			remove_action( 'admin_notices', array( $this, 'mp_core_plugin_check_notice')  );
